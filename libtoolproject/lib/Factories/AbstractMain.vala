...
using Bootstrapingproject.System;
...
namespace Bootstrapingproject.Factories
{
	...
	public abstract class AbstractMain : GLib.Object
	{
		...
		protected virtual void initialize_program ()
		{
			...
			Logger.initialize ("bootstrapingproject");
		...
		protected virtual bool initialize_libraries (ref unowned string[] args)
		{
			...
			application = new Gtk.Application ("testing.bootstrapingproject", ApplicationFlags.FLAGS_NONE);
			try {
				if (application.register () && !application.get_is_remote ())
					return true;
			} catch (Error e) {
				critical ("Registering application as '%s' failed. (%s)", "bootstrapingproject", e.message);
				return false;
			}
